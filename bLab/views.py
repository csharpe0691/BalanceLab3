...
def single_query(request, first_name, last_name, gender, age1, age2, search_word_1, search_word_2, search_word_3, search_word_4, search_word_5):
    ...
    if finalQueryDict1.count() == 0:
        ...
        table_data = create_dummy_table_data()
        ...
        finalQueryDf1 = pd.DataFrame.from_records(finalQueryDict1)
        print(finalQueryDf1)
        query1SingleRow = finalQueryDf1.loc[0]
        print(query1SingleRow)

        BECAREA = query1SingleRow['BECAREA']
        print('BECAREA:', BECAREA)
    ...
    if (finalQueryDict1.count() == 1):
        ...
        if (finalQueryDict2.count() == 0):
            ...
            table_data = [
                {'row_name': 'Area', 'BEO_pt': query1SingleRow['BEOAREA'], 'BEO_ctrls': finalQueryDf2['BEOAREA'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECAREA'], 'BEC_ctrls': finalQueryDf2['BECAREA'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEOAREA'], 'UEO_ctrls': finalQueryDf2['UEOAREA'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECAREA'], 'UEC_ctrls': finalQueryDf2['UECAREA'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': query1SingleRow['BNDAREA'], 'BND_ctrls': finalQueryDf2['BNDAREA'].mean(), 'BND_Pcnt': 0.0},
                {'row_name': 'Range AP', 'BEO_pt': query1SingleRow['BEORANGEAP'], 'BEO_ctrls': finalQueryDf2['BEORANGEAP'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECRANGEAP'], 'BEC_ctrls': finalQueryDf2['BECRANGEAP'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEORANGEAP'], 'UEO_ctrls': finalQueryDf2['UEORANGEAP'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECRANGEAP'], 'UEC_ctrls': finalQueryDf2['UECRANGEAP'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': query1SingleRow['BNDRANGEAP'], 'BND_ctrls': finalQueryDf2['BNDRANGEAP'].mean(), 'BND_Pcnt': 0.0},
                {'row_name': 'Range ML', 'BEO_pt': query1SingleRow['BEORANGEML'], 'BEO_ctrls': finalQueryDf2['BEORANGEML'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECRANGEML'], 'BEC_ctrls': finalQueryDf2['BEORANGEML'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEORANGEML'], 'UEO_ctrls': finalQueryDf2['UEORANGEML'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECRANGEML'], 'UEC_ctrls': finalQueryDf2['UECRANGEML'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': query1SingleRow['BNDRANGEML'], 'BND_ctrls': finalQueryDf2['BNDRANGEML'].mean(), 'BND_Pcnt': 0.0},
                {'row_name': 'MV AP', 'BEO_pt': 0.0, 'BEO_ctrls': 0.0, 'BEO_Pcnt': 0.0, 'BEC_pt': 0.0, 'BEC_ctrls': 0.0, 'BEC_Pcnt': 0.0, 'UEO_pt': 0.0, 'UEO_ctrls': 0.0, 'UEO_Pcnt': 0.0, 'UEC_pt': 0.0, 'UEC_ctrls': 0.0, 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
                {'row_name': 'MV ML', 'BEO_pt': 0.0, 'BEO_ctrls': 0.0, 'BEO_Pcnt': 0.0, 'BEC_pt': 0.0, 'BEC_ctrls': 0.0, 'BEC_Pcnt': 0.0, 'UEO_pt': 0.0, 'UEO_ctrls': 0.0, 'UEO_Pcnt': 0.0, 'UEC_pt': 0.0, 'UEC_ctrls': 0.0, 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
                {'row_name': 'RMS AP', 'BEO_pt': query1SingleRow['BEORMSAP'], 'BEO_ctrls': finalQueryDf2['BEORMSAP'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECRMSAP'], 'BEC_ctrls': finalQueryDf2['BECRMSAP'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEORMSAP'], 'UEO_ctrls': finalQueryDf2['UEORMSAP'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECRMSAP'], 'UEC_ctrls': finalQueryDf2['UECRMSAP'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
                {'row_name': 'RMS ML', 'BEO_pt': query1SingleRow['BEORMSML'], 'BEO_ctrls': finalQueryDf2['BEORMSML'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECRMSML'], 'BEC_ctrls': finalQueryDf2['BECRMSML'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEORMSML'], 'UEO_ctrls': finalQueryDf2['UEORMSML'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECRMSML'], 'UEC_ctrls': finalQueryDf2['UECRMSML'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
                {'row_name': 'F80 AP (Hz)', 'BEO_pt': query1SingleRow['BEOF80AP'], 'BEO_ctrls': finalQueryDf2['BEOF80AP'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECF80AP'], 'BEC_ctrls': finalQueryDf2['BECF80AP'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEOF80AP'], 'UEO_ctrls': finalQueryDf2['UEOF80AP'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECF80AP'], 'UEC_ctrls': finalQueryDf2['UECF80AP'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
                {'row_name': 'F80 ML (Hz)', 'BEO_pt': 0.0, 'BEO_ctrls': 0.0, 'BEO_Pcnt': 0.0, 'BEC_pt': 0.0, 'BEC_ctrls': 0.0, 'BEC_Pcnt': 0.0, 'UEO_pt': 0.0, 'UEO_ctrls': 0.0, 'UEO_Pcnt': 0.0, 'UEC_pt': 0.0, 'UEC_ctrls': 0.0, 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
                {'row_name': 'Sent AP', 'BEO_pt': query1SingleRow['BEOSENTAP'], 'BEO_ctrls': finalQueryDf2['BEOSENTAP'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECSENTAP'], 'BEC_ctrls': finalQueryDf2['BECSENTAP'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEOSENTAP'], 'UEO_ctrls': finalQueryDf2['UEOSENTAP'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECSENTAP'], 'UEC_ctrls': finalQueryDf2['UECSENTAP'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
                {'row_name': 'Sent ML', 'BEO_pt': query1SingleRow['BEOSENTML'], 'BEO_ctrls': finalQueryDf2['BEOSENTML'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECSENTML'], 'BEC_ctrls': finalQueryDf2['BECSENTML'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEOSENTML'], 'UEO_ctrls': finalQueryDf2['UEOSENTML'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECSENTML'], 'UEC_ctrls': finalQueryDf2['UECSENTML'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
                {'row_name': 'CrossSent', 'BEO_pt': query1SingleRow['BEOCROSSSENT'], 'BEO_ctrls': finalQueryDf2['BEOCROSSSENT'].mean(), 'BEO_Pcnt': 0.0, 'BEC_pt': query1SingleRow['BECCROSSSENT'], 'BEC_ctrls': finalQueryDf2['BECCROSSSENT'].mean(), 'BEC_Pcnt': 0.0, 'UEO_pt': query1SingleRow['UEOCROSSSENT'], 'UEO_ctrls': finalQueryDf2['UEOCROSSSENT'].mean(), 'UEO_Pcnt': 0.0, 'UEC_pt': query1SingleRow['UECCROSSSENT'], 'UEC_ctrls': finalQueryDf2['UECCROSSSENT'].mean(), 'UEC_Pcnt': 0.0, 'BND_pt': 0.0, 'BND_ctrls': 0.0, 'BND_Pcnt': 0.0},
            ]
